...
-(NSString *)ReadStringValue:(int)timeout
{
    ...
    while (1) {
        ...
        @synchronized(self.dataInput)
        {
            NSString *str = [[NSString alloc]initWithData:self.dataInput encoding:NSASCIIStringEncoding];
            if (str && [str length]>0)
            {
                strData=[NSString stringWithString:str];
                [str release];
...
- (void)serialPortReadData:(NSDictionary *)dataDictionary
{
	...
	if (data && [data length] > 0) {
        if (m_SerialView)
        {
            NSMutableData* DataRecv = [NSMutableData dataWithBytes:"recv :" length:strlen("recv :")];
            [DataRecv appendData:data];
            [m_SerialView InsertData:DataRecv];
        }
        
        if (m_CallbackTarget)
        {
            [m_CallbackTarget performSelectorInBackground:m_SerialCallBack withObject:data];
        }
