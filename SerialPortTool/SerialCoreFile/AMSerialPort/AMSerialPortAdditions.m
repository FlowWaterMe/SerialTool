...
//                            break;
                ...
                if ([data length])
                {
                    [delegate performSelectorOnMainThread:@selector(serialPortReadData:)
                                               withObject:[NSDictionary dictionaryWithObjectsAndKeys: self, @"serialPort", [NSData dataWithData: data], @"data", nil]
                                            waitUntilDone:NO];
                }
        ...
        {
            NSLog(@"encounter exception:%@",[exception description]);
        }
        @finally
        {
            [localAutoreleasePool drain];
            localAutoreleasePool=nil;
